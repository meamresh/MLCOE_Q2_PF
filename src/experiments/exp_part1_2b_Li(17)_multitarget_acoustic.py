...
Usage:
    python -m "src.experiments.exp_part1_2b_Li(17)_multitarget_acoustic" --gpu
    python -m "src.experiments.exp_part1_2b_Li(17)_multitarget_acoustic" --gpu --gpu_id 1
    python -m "src.experiments.exp_part1_2b_Li(17)_multitarget_acoustic" --cpu
"""
...
from src.filters.pfpf_dai_daum import PFPFDaiDaumFilter, SPFConfig
...
def plot_ess_vs_steps(summary, filters, n_steps, out_path, show_std=False):
    ...
    ess_filters = ['pf', 'pfpf_ledh', 'pfpf_edh', 'pfpf_dai_daum', 'ledh', 'edh']
...
def plot_rmse_vs_steps(summary, filters, n_steps, out_path, show_std=False):
    ...
    colors = {
        ...
        'pfpf_ledh': 'purple', 'pfpf_edh': 'orange', 'pfpf_dai_daum': 'brown',
...
def run_single_trajectory(
    ...
    for filter_name in filters:
        ...
        elif filter_name == 'pfpf_dai_daum':
            cfg = SPFConfig(
                n_particles=500,
                lambda_steps=100,           # fewer λ-steps (Hutchinson trace, cheap)
                bvp_mesh_points=50,
                bvp_max_iter=1000,
                bvp_learning_rate=1e-3,
                mu_steps=3,
                verbose=True,
            )
            fobj = PFPFDaiDaumFilter(
                ssm_filter, x0, P0,
                num_particles=500,
                config=cfg,
                resample_threshold=0.5,
                homotopy_mode='optimal',          # fast: β(λ)=λ, no BVP
                show_progress=show_inner_progress,
                reuse_beta_across_steps=False,
            )
        else:
            raise ValueError(f'Unknown filter: {filter_name}')

        store_particles = store_particles_global and filter_name in [
            'pf', 'pfpf_ledh', 'pfpf_edh', 'pfpf_dai_daum', 'ledh', 'edh'
        ]
...
def run_monte_carlo(args):
    ...
    ess_filters = [f for f in args.filters if f in ['pf', 'pfpf_ledh', 'pfpf_edh', 'pfpf_dai_daum', 'ledh', 'edh']]
    if ess_filters:
...
# python -m "src.experiments.exp_part1_2b_Li(17)_multitarget_acoustic" --n_trajectories 1 --show_inner_progress --output_dir 'reports/4_Stochastic_Particle_Flow/pfpf_comparison/' --filters pfpf_dai_daum pfpf_ledh --n_runs 3 --n_steps 10 --plot_std
