...
from src.filters.bonus.differentiable_pfpf_ledh import DifferentiablePFPFLEDHLogLikelihood
...
from src.filters.bonus.hmc_tfp import run_hmc_tfp, run_hmc_tfp_multi_chain
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        choices=("custom", "tfp"),
        default="custom",
        help="HMC implementation to use: custom leapfrog sampler or TFP HMC.",
    )
    p.add_argument(
        "--tfp_progress_every",
        type=int,
        default=10,
        help="Print TFP HMC progress every N iterations (0 = fused sample_chain, no step logs).",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    tfp_progress_every = int(args.tfp_progress_every)
    ...
    out_dir = Path(
        f"reports/6_BonusQ1_HMC_Invertible_Flows/HMC_vs_PMMH/hmc_ledh_onlyNEW_legacy_{sampler}"
    )
    ...
    print(f"  T={T}  N_ledh={N_ledh}  L={L}  sampler={sampler}  adapt_mass_matrix={adapt_mass}")
    ...
    if sampler == "tfp":
        print("  TFP CRN: fixed target realisation per chain")
        print(f"  TFP progress: every {tfp_progress_every} iter(s)" if tfp_progress_every > 0 else "  TFP progress: off (fused chain)")
    print("=" * 70)
    ...
    ledh_filter = DifferentiablePFPFLEDHLogLikelihood(
    ...
    def ledh_ll(sigma_v_sq, sigma_w_sq, y):
    ...
    if chain_id >= 0:
        ...
        if sampler == "tfp":
            result = run_hmc_tfp(
                ...
                crn_seed=base_seed,
                progress_every=tfp_progress_every,
            )
        else:
            result = run_hmc(
                target_log_prob_fn=target_hmc,
    ...
    print("\n" + "=" * 70)
    print(f"  Running HMC-LEDH ({num_chains} chains in one process, sampler={sampler})")
    print("=" * 70)
    t0 = time.time()
    if sampler == "tfp":
        mc = run_hmc_tfp_multi_chain(
            target_log_prob_fn=target_hmc,
