...
def _plot_tradeoff_results(results: list[dict], out_dir=None) -> None:
    ...
    iters = np.array([r["iters"] for r in results], dtype=np.int32)
    bias = np.array([r["bias"] for r in results], dtype=np.float64)
    std = np.array([r["std"] for r in results], dtype=np.float64)
    time_ms = np.array([r["time_ms"] for r in results], dtype=np.float64)

    unique_iters = sorted(set(int(x) for x in iters.tolist()))
    marker_cycle = ["o", "s", "^", "D", "x", "*", "P", "v"]
    marker_map = {k: marker_cycle[i % len(marker_cycle)] for i, k in enumerate(unique_iters)}

    vmin = float(np.min(eps))
    vmax = float(np.max(eps))

    def _scatter(ax, x, y, x_label, y_label, title, fname):
        last_scatter = None
    ...
    _scatter(
        ...
        x=bias,
        y=std,
        x_label="Bias (mean ELBO - true log-likelihood)",
        y_label="Std (variance estimate)",
        title="DPF bias-variance trade-off",
        fname="bias_vs_std.png",
    )
    ...
    _scatter(
        ...
        y=time_ms,
        x_label="Bias (mean ELBO - true log-likelihood)",
        y_label="Avg runtime (ms per run)",
        title="DPF bias vs runtime",
        fname="bias_vs_time_ms.png",
    )
    ...
    _scatter(
        ...
        x=std,
        y=time_ms,
        x_label="Std (variance estimate)",
        y_label="Avg runtime (ms per run)",
        title="DPF std vs runtime",
        fname="std_vs_time_ms.png",
    )
