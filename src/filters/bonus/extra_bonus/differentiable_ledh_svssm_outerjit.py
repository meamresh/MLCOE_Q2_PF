...
class DifferentiableLEDHLogLikelihoodSVSSMOuterJIT:
    ...
    def __init__(
        ...
        grad_window: int = 4,
        jit_compile: bool = True,             # per-step body JIT (default behaviour)
        integrator: str = "exp",
        log_y_sq_offset: float = 1e-8,
        init_type: str = "stationary",
        diffuse_var: float = 100.0,
        outer_jit: bool = False,              # wrap the WHOLE outer loop in tf.function(jit_compile=True)
    ):
        ...
        # If outer_jit, wrap the outer loop once at construction time so the
        # same trace is reused across calls. Per-step body is still JIT'd
        # individually inside, which XLA treats as inlinable.
        if self.outer_jit:
            self._run_dispatch = tf.function(self._run_1d_xla, jit_compile=True)
        else:
            self._run_dispatch = None  # use bound method directly via the flag below
    ...
    def __call__(
        ...
        if self.outer_jit:
            log_ml = self._run_dispatch(mu_t, phi_t, sigma_eta_sq_t, z_obs, init_var, init_mean)
        elif self.jit_compile:
