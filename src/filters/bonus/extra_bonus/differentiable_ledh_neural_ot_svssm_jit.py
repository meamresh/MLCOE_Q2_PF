...
class DifferentiableLEDHNeuralOTSVSSMJIT:
    ...
    def __init__(
        ...
        grad_window: int = 4,
        jit_compile: bool = False,           # legacy flag, kept for API compat
        integrator: str = "exp",
        log_y_sq_offset: float = 1e-8,
        init_type: str = "stationary",
        diffuse_var: float = 100.0,
        graph_mode: str = "eager",            # "eager" | "graph" | "xla"
    ):
        ...
        if graph_mode not in {"eager", "graph", "xla"}:
            raise ValueError(
                f"graph_mode must be 'eager', 'graph', or 'xla'; got {graph_mode!r}"
            )
        ...
        # Wrap _run_1d at construction time so each call uses the same trace.
        if graph_mode == "graph":
            self._run_1d_dispatch = tf.function(self._run_1d)
        elif graph_mode == "xla":
            self._run_1d_dispatch = tf.function(self._run_1d, jit_compile=True)
        else:
            self._run_1d_dispatch = self._run_1d
    ...
    def __call__(
        ...
        log_ml = self._run_1d_dispatch(mu_t, phi_t, sigma_eta_sq_t, z_obs, init_var, init_mean)
